...
SUM_WEIGHT = ROI_WEIGHT + STRATEGY_SELL_WEIGHT + TRAILING_WEIGHT + WIN_WEIGHT + MIN_STOP_LOSS_WEIGHT + PROFIT_WEIGHT
...
class ROIAndProfitHyperOptLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(results: DataFrame, trade_count: int, *args, **kwargs) -> float:
        # Calculate the rate for diferent sell reason types
        results.loc[
            (results['sell_reason'] == 'roi'),
            'roi_signals'
        ]=1
        roi_signals_rate = results['roi_signals'].sum() / trade_count

        results.loc[
            (results['sell_reason'] == 'sell_signal'),
